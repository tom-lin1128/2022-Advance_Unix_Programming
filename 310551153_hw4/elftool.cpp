...
void load_phdr(elfhandle_t* e){
    if(e->ph_cnt == 0)
        return;
    e->phdr = (Elf64_Phdr*)calloc(1, e->ehdr->e_phentsize * e->ehdr->e_phnum); //the product of e_phentsize and e_phnum gives the section header table's size in bytes
...
void load_shdr(elfhandle_t* e){
    if(e->sh_cnt == 0)
        return;
    e->shdr = (Elf64_Shdr*)calloc(1, e->ehdr->e_shentsize * e->ehdr->e_shnum); //the product of e_shentsize and e_shnum gives the section header table's size in bytes
